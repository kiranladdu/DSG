package com.example.dsg.permission;
...
public class PermissionUtils {
    ...
    PermissionResultCallback mPermissionResultCallback;
            ...
            case 1:
                if (grantResults.length > 0) {
                    ...
                    for (int i = 0; i < listPermissionsNeeded.size(); i++) {
                        if (perms.get(listPermissionsNeeded.get(i)) != PackageManager.PERMISSION_GRANTED) {
                            if (ActivityCompat.shouldShowRequestPermissionRationale(current_activity, listPermissionsNeeded.get(i)))
                                ...
                                mPermissionResultCallback.NeverAskAgain(req_code);
                    ...
                    if (pending_permissions.size() > 0) {
                                        ...
                                        switch (which) {
                                            ...
                                            case DialogInterface.BUTTON_NEGATIVE:
                                                ...
                                                if (permission_list.size() == pending_permissions.size())
                                                    mPermissionResultCallback.PermissionDenied(req_code);
                                                else
                                                    mPermissionResultCallback.PartialPermissionGranted(req_code, pending_permissions);
                                                break;
                                        }
                        ...
                        mPermissionResultCallback.PermissionGranted(req_code);
